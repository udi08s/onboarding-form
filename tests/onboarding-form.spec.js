...
test.describe("Onboarding Form", () => {
  ...
  test.describe("Positive Scenarios", () => {
    ...
    test("should submit form successfully with all required fields", async ({
      page,
    }) => {
    ...
    test("should submit form successfully including optional company field", async ({
      page,
    }) => {
      // Fill personal information (required fields)
      ...
      await onboardingForm.fillCompany(
        testData.validFormDataWithCompany.company,
      );

      // Select dropdown options
      await onboardingForm.selectState(testData.validFormDataWithCompany.state);
      await onboardingForm.selectGender(
        testData.validFormDataWithCompany.gender,
      );
    ...
    test.describe("Boundary Value Testing", () => {
      test("should accept minimum valid values", async ({ page }) => {
        ...
        // Select dropdown options
        await onboardingForm.selectState(
          testData.boundaryFormDataMinimum.state,
        );
        await onboardingForm.selectGender(
          testData.boundaryFormDataMinimum.gender,
        );
      ...
      test("should accept maximum valid values", async ({ page }) => {
        ...
        await onboardingForm.fillCompany(
          testData.boundaryFormDataMaximum.company,
        );

        // Select dropdown options
        await onboardingForm.selectState(
          testData.boundaryFormDataMaximum.state,
        );
        await onboardingForm.selectGender(
          testData.boundaryFormDataMaximum.gender,
        );
  ...
  test.describe("Negative Scenarios", () => {
    test.describe("Required Field Validations", () => {
      test("should show error messages for empty form submission", async ({
        page,
      }) => {
      ...
      test("should show error for missing first name", async ({ page }) => {
        ...
        await onboardingForm.fillPersonalInfo(
          negativeTestData.missingFirstName,
        );
        await onboardingForm.selectState(
          negativeTestData.missingFirstName.state,
        );
        await onboardingForm.selectGender(
          negativeTestData.missingFirstName.gender,
        );
      ...
      test("should show error for missing last name", async ({ page }) => {
        ...
        await onboardingForm.selectState(
          negativeTestData.missingLastName.state,
        );
        await onboardingForm.selectGender(
          negativeTestData.missingLastName.gender,
        );
      ...
      test("should show error for missing date of birth", async ({ page }) => {
        ...
        await onboardingForm.fillPersonalInfo(
          negativeTestData.missingDateOfBirth,
        );
        await onboardingForm.selectState(
          negativeTestData.missingDateOfBirth.state,
        );
        await onboardingForm.selectGender(
          negativeTestData.missingDateOfBirth.gender,
        );
    ...
    test.describe("First Name & Last Name Minimum Validation", () => {
      test("should show minimum character error for single character first name", async ({
        page,
      }) => {
        // Fill form with single character first name
        await onboardingForm.fillPersonalInfo(
          negativeTestData.singleCharacterFirstName,
        );
        await onboardingForm.selectState(
          negativeTestData.singleCharacterFirstName.state,
        );
        await onboardingForm.selectGender(
          negativeTestData.singleCharacterFirstName.gender,
        );
      ...
      test("should show minimum character error for single character last name", async ({
        page,
      }) => {
        // Fill form with single character last name
        await onboardingForm.fillPersonalInfo(
          negativeTestData.singleCharacterLastName,
        );
        await onboardingForm.selectState(
          negativeTestData.singleCharacterLastName.state,
        );
        await onboardingForm.selectGender(
          negativeTestData.singleCharacterLastName.gender,
        );
    ...
    test.describe("Email Validation", () => {
      test("should show format error for email without dot in domain", async ({
        page,
      }) => {
        // Fill form with invalid email format (no dot in domain)
        await onboardingForm.fillPersonalInfo(
          negativeTestData.invalidEmailNoDot,
        );
        await onboardingForm.selectState(
          negativeTestData.invalidEmailNoDot.state,
        );
        await onboardingForm.selectGender(
          negativeTestData.invalidEmailNoDot.gender,
        );
    ...
    test.describe("Phone Validation", () => {
      const invalidPhoneNumbers =
        negativeTestData.invalidPhoneFormats.phoneNumbers;

      invalidPhoneNumbers.forEach((phoneNumber) => {
        test(`should show format error for invalid phone: ${phoneNumber}`, async ({
          page,
        }) => {
          // Fill form with invalid phone number
          const testData = {
            ...negativeTestData.invalidPhoneFormats,
            phone: phoneNumber,
          };
    ...
    test.describe("Date of Birth Validation", () => {
      test("should show error for future date of birth", async ({ page }) => {
        ...
        await onboardingForm.fillPersonalInfo(
          negativeTestData.futureDateOfBirth,
        );
        await onboardingForm.selectState(
          negativeTestData.futureDateOfBirth.state,
        );
        await onboardingForm.selectGender(
          negativeTestData.futureDateOfBirth.gender,
        );
      ...
      test("should show error for too young age (under 13)", async ({
        page,
      }) => {
        // Fill form with date making user under 13 years old
        await onboardingForm.fillPersonalInfo(
          negativeTestData.tooYoungDateOfBirth,
        );
        await onboardingForm.selectState(
          negativeTestData.tooYoungDateOfBirth.state,
        );
        await onboardingForm.selectGender(
          negativeTestData.tooYoungDateOfBirth.gender,
        );
      ...
      test("should show error for too old age (over 120)", async ({ page }) => {
        ...
        await onboardingForm.fillPersonalInfo(
          negativeTestData.tooOldDateOfBirth,
        );
        await onboardingForm.selectState(
          negativeTestData.tooOldDateOfBirth.state,
        );
        await onboardingForm.selectGender(
          negativeTestData.tooOldDateOfBirth.gender,
        );
    ...
    test.describe("Company Field Validation", () => {
      test("should show minimum character error for single character company name", async ({
        page,
      }) => {
        // Fill form with single character company name
        await onboardingForm.fillPersonalInfo(
          negativeTestData.singleCharacterCompany,
        );
        await onboardingForm.fillCompany(
          negativeTestData.singleCharacterCompany.company,
        );
        await onboardingForm.selectState(
          negativeTestData.singleCharacterCompany.state,
        );
        await onboardingForm.selectGender(
          negativeTestData.singleCharacterCompany.gender,
        );
